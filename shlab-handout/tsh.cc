...
void eval(char *cmdline) 
{
  ...
  if (sigfillset(&mask_all) < 0) {
     unix_error("maskfill error"); 
  }
  if (sigemptyset(&mask_one) < 0) {
      unix_error("maskempty error");
  }
  if (sigaddset(&mask_one, SIGCHLD) < 0) {
      unix_error("maskadd SIGCHLD error");
  }
...
void sigchld_handler(int sig) 
{
  ...
  if(errno != ECHILD && pid < 0) {
      unix_error("waitpid error in sigchild_handler");
  }
  errno = olderrno;
  return;
}
...
void sigint_handler(int sig) 
{
  if(getjobpid(jobs, fgpid(jobs)) == NULL) {
      return;
  }
  kill(-fgpid(jobs), sig);
}
